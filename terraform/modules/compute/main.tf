resource "aws_alb" "ecs_alb" {
  ...
  access_logs {
    bucket = data.terraform_remote_state.global.outputs.alb_logs_bucket
...
resource "aws_launch_configuration" "ecs_lc" {
  key_name             = data.terraform_remote_state.global.outputs.ssh_key
  name_prefix          = "${var.env}-ecs-instance"
  image_id             = data.aws_ssm_parameter.ecs_optimized_image_id.value
  instance_type        = var.instance_type
  iam_instance_profile = data.terraform_remote_state.global.outputs.ecs_instance_role_name
